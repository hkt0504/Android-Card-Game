...
bool UserScene::init()
{
	...
	sprGroup1->setPosition(deltaX, deltaY);
	this->addChild(sprGroup1, 1);

	Sprite *userImg = Sprite::create("avatars/sample_user.png");
	userImg->setPosition(deltaX - sprGroup1->getContentSize().width / 4 - userImg->getContentSize().width / 2, deltaY);
	addChild(userImg, 2, IMG_USER_AVATAR_TAG);

	LabelTTF *labelUser = LabelTTF::create("UserName", "Felt", 18);
	labelUser->setPosition(deltaX + sprGroup1->getContentSize().width / 8, deltaY + sprGroup1->getContentSize().height / 4);
	addChild(labelUser, 2, LABEL_USER_NAME_TAG);

	LabelTTF *labelLevel = LabelTTF::create("Level 0", "Felt", 18);
	labelLevel->setPosition(deltaX - 20, deltaY - sprGroup1->getContentSize().height / 4 + 2);
	...
	starItem->setPosition(deltaX, deltaY);
	...
	sprGroup3->setPosition(deltaX,	deltaY);
	this->addChild(sprGroup3, 1);

	LabelTTF *labelWin = LabelTTF::create("Win : 0", "Felt", 18);
	labelWin->setPosition(deltaX - labelWin->getContentSize().width / 2 - 12,
	...
	labelLose->setPosition(deltaX + labelLose->getContentSize().width - 5,
			deltaY - sprGroup3->getContentSize().height / 4 + 2);
	...
	settingItem->setPosition(deltaX, deltaY);
	...
	auto playItem = MenuItemImage::create("Buttons/play_now.png", "Buttons/play_now.png",
	...
	playItem->setPosition(deltaX, deltaY);
	...
	auto shareItem = MenuItemImage::create("Buttons/share_game.png", "Buttons/share_game.png",
			CC_CALLBACK_1(UserScene::menuShareGame, this));
	shareItem->setPosition(deltaX, deltaY);
	...
	auto inviteItem = MenuItemImage::create("Buttons/invite_friends.png", "Buttons/invite_friends.png",
			CC_CALLBACK_1(UserScene::menuInviteFriend, this));
	inviteItem->setPosition(deltaX, deltaY);
	...
	buyItem->setPosition(deltaX, deltaY);
...
{
	Scene *pScene = TransitionFade::create(0.5, GameScene::createScene(GAME_FROM_FACEBOOK));
	Director:: getInstance()->replaceScene(pScene);
}
