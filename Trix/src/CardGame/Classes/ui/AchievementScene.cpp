...
bool AchievementScene::init()
{
	...
	auto boardBg = Sprite::create("other/achiv_board.png");
	boardBg->setPosition(origin.x + visibleSize.width / 2, origin.y + visibleSize.height / 2 - 50);
...
{
	auto junior = Sprite::create((win >= JUNIOR) ? "other/achiv_junior.png" : "other/achiv_junior_d.png");
	junior->setAnchorPoint(Vec2::ZERO);
	junior->setPosition(boardOrigin.x + 30, boardOrigin.y + 207);
	addChild(junior, 3);

	auto senior = Sprite::create((win >= SENIOR) ? "other/achiv_senior.png" : "other/achiv_senior_d.png");
	senior->setAnchorPoint(Vec2::ZERO);
	senior->setPosition(boardOrigin.x + 530, boardOrigin.y + 207);
	addChild(senior, 3);

	auto excellent = Sprite::create((win >= EXCELLENT) ? "other/achiv_excellent.png" : "other/achiv_excellent_d.png");
	excellent->setAnchorPoint(Vec2::ZERO);
	excellent->setPosition(boardOrigin.x + 30, boardOrigin.y + 117);
	addChild(excellent, 3);

	auto talent = Sprite::create((win >= TALENT) ? "other/achiv_talent.png" : "other/achiv_talent_d.png");
	talent->setAnchorPoint(Vec2::ZERO);
	talent->setPosition(boardOrigin.x + 550, boardOrigin.y + 117);
	addChild(talent, 3);

	auto genius = Sprite::create((win >= GENIUS) ? "other/achiv_genius.png" : "other/achiv_genius_d.png");
	genius->setAnchorPoint(Vec2::ZERO);
	genius->setPosition(boardOrigin.x + 30, boardOrigin.y + 30);
	addChild(genius, 3);
}
